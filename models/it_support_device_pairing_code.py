...
class ItSupportDevicePairingCode(models.Model):
    ...
    def create_for_partner(self, partner):
        """One active code per requester at a time - generating a new one silently
        invalidates any earlier unused code instead of letting several pile up."""
        self.search([('partner_id', '=', partner.id), ('used', '=', False)]).write({'used': True})
        return self.create({
            'code': self._generate_code(),
            'partner_id': partner.id,
            'customer_id': partner.commercial_partner_id.id,
            'expires_at': fields.Datetime.now() + timedelta(minutes=CODE_VALIDITY_MINUTES),
        })
